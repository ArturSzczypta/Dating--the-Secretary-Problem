...
test_name = f'pop {population} cy {cycles} min {min_val} max {max_val}' \
            f' imp {improvement} upper {max_posible} bottom {min_posible}.csv'
...
def single_run() -> np.ndarray:
    ...
    if improvement != 1:
        ...
        if max_val * improvement > max_posible:
            filler[filler > max_posible] = max_posible
        if max_val * improvement < min_posible:
            filler[filler < min_posible] = min_posible
